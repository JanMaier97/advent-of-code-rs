...
fn solve_with_input(input: &str, dim: Dimensions, times: u64) -> MyResult<u64> {
    ...
    let points = robots.iter().map(|r| move_robot(*r, dim, times))
...
fn normalize_value(value: i32, identity: u64) -> u64 {
    if value < 0 {
        ...
        if abs >= identity {
            println!("normalized {} to {}", value, identity * times + identity - rem);
        }
...
fn count_positions(poinst: &[Point<u64>], dim: Dimensions)  -> usize {
    ...
    let top_left = poinst.iter().filter(|r| r.x < middle_x && r.y < middle_y ).count();
    let top_right = poinst.iter().filter(|r| r.x > middle_x && r.y < middle_y ).count();
    let bottom_right = poinst.iter().filter(|r| r.x > middle_x && r.y > middle_y ).count();
    let bottom_left = poinst.iter().filter(|r| r.x < middle_x && r.y > middle_y ).count();
